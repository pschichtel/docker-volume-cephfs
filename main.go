...
type cephfsDriver struct {
	...
	root      string
	statePath string
	volumes   map[string]*cephfsVolume
}
...
func newCephfsDriver(root string) (*cephfsDriver, error) {
	...
	d := &cephfsDriver{
		...
		statePath: filepath.Join(root, "state", "cephfs-state.json"),
		volumes:   map[string]*cephfsVolume{},
	}
...
func (d *cephfsDriver) Create(r *volume.CreateRequest) error {
	...
	v.Mountpoint = filepath.Join(d.root, u+"-"+fmt.Sprintf("%x", md5.Sum([]byte(v.Path))))
...
func (d *cephfsDriver) mountVolume(v *cephfsVolume) error {
	...
	if err != nil {
		return logError("cephfs command execute failed: %v (%s)", err, output)
	}
	return nil
}
